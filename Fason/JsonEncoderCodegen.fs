...
type JsonEncoderCodegen =
    ...
    static member private getTypeName(typ: SerializableType ref) =
        ...
        | SerializableType.Map(k, v) -> %($"Map<{k |> JsonEncoderCodegen.getTypeName}, {v |> JsonEncoderCodegen.getTypeName}>")

        | SerializableType.UnitOfMeasure uom -> %($"{uom.baseType |> JsonEncoderCodegen.getTypeName}<{uom.unitOfMeasure}>")
        ...
        MatchExpr(
            ...
            [
                for value in typ.values do
                    MatchClauseExpr($"{typ.name}.{value.name}", AppExpr("writer.WritePlain", EscapedString $"\"{value.name}\""))

                MatchClauseExpr("other", AppExpr("JsonSerializer.serialize", ParenExpr(TupleExpr([ $"{baseTypeName} other"; "writer" ]))))
            ]
        )
    ...
    static member private generateFieldDeserializer(fields: RecordField list, typeName: string<typeName>, isAnonymous: bool) =
        CompExprBodyExpr [
            ...
            OtherExpr(
                IfThenExpr("reader.ReadUntil(\"{\") |> not", AppExpr("failwith", EscapedString "expected the start of an object, but got something else"))
            )
                ...
                let fieldReader (field, index) =
                    ...
                    CompExprBodyExpr [
                        ...
                        IfThenExpr(
                            ...
                            AppExpr("failwith", EscapedString "expected a colon after the field name, but got something else")
                        )
                ...
                let mainIfBody =
                    ...
                    CompExprBodyExpr [
                        ...
                        | true, false ->
                            ...
                            IfThenElseExpr("reader.Peek() = Some '\"'", fieldReader (field, index), AppExpr("reader.SkipObjectField", "()"))
                        ...
                        IfThenElifExpr(
                            [
                                ...
                                ElIfThenExpr(
                                    ...
                                    CompExprBodyExpr [
                                        IfThenExpr(
                                            "reader.ReadUntil(\",\") |> not",
                                            AppExpr("failwith", EscapedString "expected a comma after the field value, but got something else")
                                        )
                            ...
                            CompExprBodyExpr [
                                ...
                                OtherExpr(
                                    AppExpr(
                                        "failwith",
                                        EscapedString "expected a quote before a field name, or an object closing brace, but instead got something else"
                                    )
                                )
                            ]
                        )
    ...
    static member private generateAnonymousRecordDeserializer(typ: AnonymousRecordType, typeName: string<typeName>) =
        CompExprBodyExpr [
            JsonEncoderCodegen.generateFieldDeserializer (typ.fields, typeName, isAnonymous = true)
        ]

    static member private generateRecordDeserializer(typ: RecordType, typeName: string<typeName>) =
        CompExprBodyExpr [
            JsonEncoderCodegen.generateFieldDeserializer (typ.fields, typeName, isAnonymous = false)
        ]
    ...
    static member private generateUnionCaseDeserializer(case: UnionCase, typeName: string<typeName>) =
        if case.fields.Length = 0 then
            ...
            CompExprBodyExpr [
                ...
                OtherExpr(
                    IfThenExpr("not inArray", AppExpr("failwith", EscapedString "tried reading a union case with values, but the tag was not in an array"))
                )

                OtherExpr(
                    IfThenExpr("reader.ReadUntil(\",\") |> not", AppExpr("failwith", EscapedString "expected a comma after the tag, but got something else"))
                )
                ...
                for i in 0 .. case.fields.Length - 1 do
                    ...
                    LetOrUseExpr(Value(NamedPat($"u{i}"), JsonEncoderCodegen.parserForType (field.fieldType, withOptional = false)))

                OtherExpr(AppExpr($"{typeName}.{case.name}", ParenExpr(TupleExpr(case.fields |> List.mapi (fun i _ -> $"u{i}") |> List.map IdentExpr))))
            ]
        ...
        CompExprBodyExpr [
            ...
            OtherExpr(
                IfThenExpr(
                    ...
                    IfThenExpr(
                        "reader.ReadUntil(\"]\") |> not",
                        AppExpr("failwith", EscapedString "expected a closing brace after the array, but got something else")
                    )
                )
            )
    ...
    static member private generateTupleDeserializer(typ: TupleType) =
        CompExprBodyExpr [
            ...
            OtherExpr(IfThenExpr("reader.ReadUntil(\"[\") |> not", AppExpr("failwith", EscapedString "expected an opening brace, but got something else")))
            ...
            for i in 0 .. typ.values.Length - 1 do
                if i > 0 then
                    OtherExpr(
                        IfThenExpr(
                            "reader.ReadUntil(\",\") |> not",
                            AppExpr("failwith", EscapedString "expected a comma after the tuple value, but got something else")
                        )
                    )

                LetOrUseExpr(Value(NamedPat($"t{i}"), JsonEncoderCodegen.parserForType (typ.values[i].valueType, withOptional = false)))

            OtherExpr(IfThenExpr("reader.ReadUntil(\"]\") |> not", AppExpr("failwith", EscapedString "expected a closing brace, but got something else")))
                ...
                WhileExpr(
                    ...
                    CompExprBodyExpr [
                        ...
                        IfThenExpr(
                            ...
                            IfThenExpr(
                                "reader.ReadUntil(\",\") |> not",
                                AppExpr("failwith", EscapedString "expected a comma after the array value, but got something else")
                            )
                        )
                    ...
                    CompExprBodyExpr [
                        OtherExpr(
                            IfThenExpr(
                                ...
                                IfThenExpr(
                                    "reader.ReadUntil(\",\") |> not",
                                    AppExpr("failwith", EscapedString "expected a comma after the map value, but got something else")
                                )
                            )
                        )
                        OtherExpr(
                            IfThenExpr(
                                "reader.ReadUntil(\"[\") |> not",
                                AppExpr("failwith", EscapedString "expected an opening brace for the map entry, but got something else")
                            )
                        )
                        LetOrUseExpr(Value(NamedPat("key"), JsonEncoderCodegen.parserForType (fst typ, withOptional = false)))
                        OtherExpr(
                            IfThenExpr(
                                "reader.ReadUntil(\",\") |> not",
                                AppExpr("failwith", EscapedString "expected a comma after the map key, but got something else")
                            )
                        )
                        LetOrUseExpr(Value(NamedPat("value"), JsonEncoderCodegen.parserForType (snd typ, withOptional = false)))
                        OtherExpr(
                            IfThenExpr(
                                "reader.ReadUntil(\"]\") |> not",
                                AppExpr("failwith", EscapedString "expected a closing brace for the map entry, but got something else")
                            )
                        )
    ...
    static member private generateUnitOfMeasureDeserializer(typ: UomType) =
        CompExprBodyExpr [
            AppExpr("%", $"(JsonDeserializer.deserialize (TypeIdentifier<{typ.baseType |> JsonEncoderCodegen.getTypeName}>(), reader))")
        ]
    ...
    static member private generateOptionalDeserializer(typ: SerializableType ref) =
        CompExprBodyExpr [
            ...
            IfThenElseExpr(
                "reader.Peek() = Some 'n'",
                CompExprBodyExpr [
                    IfThenExpr("reader.ReadUntil(\"null\") |> not", AppExpr("failwith", EscapedString "expected null or a value, but got something else"))
    ...
    static member private optimize(oak: Fantomas.Core.SyntaxOak.Oak) =
                    ...
                    | ComputationExpressionStatement.OtherStatement(Expr.App first) :: ComputationExpressionStatement.OtherStatement(Expr.App second) :: rest ->
                        ...
                        | Expr.Constant(Constant.FromText firstFn), Expr.Constant(Constant.FromText secondFn) when
                            ...
                            let firstArg, firstRange =
                                ...
                                | _ -> failwith $"Expected single argument to writer.WritePlain, but got {first.Arguments}"

                            let secondArg =
                                match second.Arguments with
                                | [ Expr.Constant(Constant.FromText x) ] -> x.Text
                                | _ -> failwith $"Expected single argument to writer.WritePlain, but got {second.Arguments}"
        ...
        Fantomas.Core.SyntaxOak.Oak(oak.ParsedHashDirectives |> optimizeNodes, oak.ModulesOrNamespaces |> optimizeNodes, oak.Range)
        ...
        let typesWithSerializer =
            ...
            |> Seq.map (fun typ ->
                ...
                | SerializableType.Map(k, v) ->
                    ...
                    | SerializableType.UnitOfMeasure uk, SerializableType.UnitOfMeasure uv -> SerializableType.Map(uk.baseType, uv.baseType)
        ...
        let fsiOak =
            Ast.Oak() {
                Namespace("Fason") {
                    TypeDefn("JsonSerializer") {
                        for typ in typesWithSerializer do
                            JsonEncoderCodegen.generateTypeSerializer (ref typ, fsi = true)
                    }
                    ...
                    TypeDefn("JsonDeserializer") {
                        for typ in typesWithSerializer do
                            JsonEncoderCodegen.generateTypeDeserializer (ref typ, fsi = true)
                    }
                }
            }
            |> Gen.mkOak
